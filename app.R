...
### Objetos para MATCH
PJ_ID <- c("\\b.INC\\b|\\bINC\\b|\\bSL\\b|\\bSPA\\b|'A.V.V'|B V|'B.V.'|BV|BV|CORP.\\b|EIRELI|GROUP|GRUPO|'INC.'|\\bL.L.C|LIMITED|\\bLLC|\\bLTD|LTDA|'S.A'|'S.A.'|'S.L'|'S.R.L'|'S/A'|CONSULTING")
...
ui <- {navbarPage(theme = shinythemes::shinytheme("spacelab"),
                  ...
                  tabPanel("Table Wider",
                           titlePanel("Transforma tabela no formato Long para o formato Wide"),
                           sidebarPanel(width = 3,
                                        h2(),
                                        tags$h5("Faça o upload do arquivo, clique em \"Carregar colunas\" e selecione a coluna de chave e as colunas que serão transpostas."),
                                        h2(),
                                        fileInput('base_long', 'Escolha o arquivo',
                                                  accept=c(".xlsx")),
                                        h2(),
                                        actionButton("choice", "Carregar colunas"),
                                        h2(),
                                        selectInput('chave', "Chave:", choices = NULL, multiple = FALSE),
                                        h2(),
                                        selectInput('colunas', "Colunas para Wide:", choices = NULL, multiple = TRUE),
                                        h2(),
                                        actionButton("processar", "Processar"),
                                        h2(),
                                        helpText("Apenas as colunas selecionadas permanecerão no arquivo"),
                                        h2(),
                                        downloadButton('Download_Wide', 'Download')),
                           mainPanel(
                             tabsetPanel(
                               tabPanel("tabela_wide", DT::dataTableOutput("tabela_wide")))))
)}
...
server <- function(input, output, session) {
  ...
  base_BR2_Socios_CDDP <- reactive({
    ...
    readxl::read_excel(input$base_BR2_CDDP$datapath, col_names = TRUE, sheet = "Adicionais")
  ...
  ### Importa tabela em formato long
  base_long <- reactive({
    
    validate(need(input$base_long != "", "Faça o upload da base bruta em .xlsx"))
    
    filein <- input$base_long
    
    if (is.null(input$base_long))
      return(NULL)
    
    readxl::read_excel(input$base_long$datapath, col_names = TRUE)
    
  })
  
  ### Colunas Long
  cols <- observeEvent(input$choice, {
    
    base <- base_long()
    
    vars <- names(base)
    
    updateSelectInput(session, "chave","Escolha a chave", choices = vars)
    updateSelectInput(session, "colunas","Escolha as colunas", choices = vars)
    
  })
  
  ### Atualiza input das colunas long
  # cols <- eventReactive(input$choice, {
  # inFile <- input$base_long
  ...
  Socios <- reactive({
    ...
    socios <- socios %>%
      mutate("MATCHING PESSOA JURÍDICA" = ifelse(grepl(PJ_ID, toupper(`NOME DO SÓCIO`)), yes = "MATCH", no = NA))
  ...
  contatos_CDDP <- reactive({
    ...
    contatos_CDDP <- base %>%
      select(QUERY, first_name, last_name, job_title, business_email, social_url) %>%
      set_names(cols) %>%
      mutate(`URL LINKEDIN PESSOAL` = `URL LINKEDIN PESSOAL` %>%
               tolower() %>%
               na_if("false") %>%
               domain(),
             `PRIMEIRO NOME` = `PRIMEIRO NOME` %>% str_remove_all("[^[:alnum:]][:space:]]") %>% str_remove_all("[[1-9]]"),
             `ULTIMO NOME` = `ULTIMO NOME` %>% str_remove_all("[^[:alnum:]][:space:]]") %>% str_remove_all("[[1-9]]"),
             `CARGO B2B` = `CARGO B2B` %>% str_remove_all("[^[:alnum:]][:space:]]"),
             NOME = paste(`PRIMEIRO NOME`, `ULTIMO NOME`),
             "MATCHING PESSOA JURÍDICA" = ifelse(grepl(PJ_ID, toupper(NOME)), yes = "MATCH", no = NA)) %>%
      select(!NOME)
  ...
  contatos_linkedin_CDDP <- reactive({
    ...
    cols <- c("DOMÍNIO", "PRIMEIRO NOME", "ULTIMO NOME", "CARGO B2B", "EMAIL", "URL LINKEDIN PESSOAL", "MATCHING PESSOA JURÍDICA")
    base <- base_LinkedIn_CDDP()
    
    contatos_linkedin_CDDP <- base %>%
      select(Query, Nome, Cargo, Email, Linkedin_contato) %>%
      mutate(Nome = Nome %>% str_remove_all("[^[:alnum:]][:space:]]") %>% str_remove_all("[[1-9]]"),
             Cargo = Cargo %>% str_remove_all("[^[:alnum:]][:space:]]"),
             "MATCHING PESSOA JURÍDICA" = ifelse(grepl(PJ_ID, toupper(Nome)), yes = "MATCH", no = NA)) %>%
      add_column("First" = NA) %>%
    ...
    contatos_linkedin_CDDP <- contatos_linkedin_CDDP %>% select(Query, First, Last, Cargo, Email, Linkedin_contato, `MATCHING PESSOA JURÍDICA`) %>%
  ...
  BR2_Socios <- reactive({
    ...
    socios <- base_BR2_Socios_CDDP()
    
    socios <- socios %>%
      mutate("MATCHING PESSOA JURÍDICA" = ifelse(grepl(PJ_ID, toupper(`Nome do sócio`)), yes = "MATCH", no = NA),
             `Nome do sócio` = `Nome do sócio` %>% str_remove_all("[^[:alnum:]][:space:]]")) %>%
      as.data.frame()
  ...
  Contatos_Apollo <- reactive({
    ...
    contatos_apollo <- base %>% select('Company',
      ...
      mutate(WEBSITE = domain(WEBSITE),
             ...
             `CARGO B2B` = toupper(`CARGO B2B`) %>% str_remove_all("[^[:alnum:]][:space:]]"), ### trata os websites e coloca cargos em maiúsculo
             NOME = paste(`PRIMEIRO NOME`, `ULTIMO NOME`),
             "MATCHING PESSOA JURÍDICA" = ifelse(grepl(PJ_ID, toupper(NOME)), yes = "MATCH", no = NA)) %>%
      select(!NOME) %>%
  ...
  Empresas_Apollo <- reactive({
    ...
    empresas_Apollo <- base %>% select('Website',
      ...
      mutate(Website = domain(Website),
             Website = str_remove(Website, "character\\(0\\)"),
             `Company Linkedin Url` = domain(`Company Linkedin Url`),
             `Company Linkedin Url` = str_remove(`Company Linkedin Url`, "character\\(0\\)"),
             `Facebook Url` = domain(`Facebook Url`),
             `Facebook Url` = str_remove(`Facebook Url`, "character\\(0\\)"),
             `Twitter Url` = domain(`Twitter Url`),
             `Twitter Url` = str_remove(`Twitter Url`, "character\\(0\\)")) %>% ### trata os websites
      as.data.frame()
  ...
  ### Transforma tabela long para wide
  tabela_wide <- eventReactive(input$processar, {
    
    long <- base_long()
    
    tabela_long <- long %>%
      select(c(input$chave, input$colunas)) %>%
      group_by_at(input$chave) %>%
      mutate("NR" = 1:n())
    
    tabela_wide <- tabela_long %>%
      pivot_wider(names_from = `NR`, values_from = c(input$colunas))
    
    colunas <- c(input$colunas)
    i <- 1
    for (i in 1:max(tabela_long$NR)) {
      
      k <- 1
      for (k in 1:length(colunas)) {
        
        if (k < length(colunas)) {
          tabela_wide <- tabela_wide %>%
            relocate(paste0(colunas[k+1], "_",i), .after = paste0(colunas[k], "_",i))
        } else {
          tabela_wide <- tabela_wide %>%
            relocate(paste0(colunas[k], "_",i), .after = paste0(colunas[k], "_",i))
        }
    ...
    names(tabela_wide) <- names(tabela_wide) %>% str_replace("_", " ")
  ...
  output$BR2_Socios_CDDP <- DT::renderDataTable(datatable(BR2_Socios(), options = list(pageLength = 1000, lengthChange = FALSE, paging = FALSE)))
  ...
  ### Pré-visualização da tabela wide
  output$tabela_wide <- DT::renderDataTable(datatable(tabela_wide()))
  ...
  output$Download_BR2_CDDP <- {downloadHandler(
    ...
    content = function(file) {
      ...
      openxlsx::writeData(wb = wb, sheet = "Sócios", x = BR2_Socios())
  ...
  output$Download_Wide <- {downloadHandler(
    ...
    content = function(file) {
      ...
      wb <- openxlsx::createWorkbook()
      openxlsx::addWorksheet(wb = wb, sheetName = "Left_JoineR")
      openxlsx::writeData(wb = wb, sheet = "Left_JoineR", x = tabela_wide())
